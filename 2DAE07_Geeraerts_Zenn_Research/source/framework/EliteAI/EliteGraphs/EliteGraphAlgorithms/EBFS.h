...
namespace Elite 
{
	...
	{
		std::queue<T_NodeType*> openList; // nodes om te bezoeken
		...
		while (!openList.empty())
		{
			...
			for (auto con : m_pGraph->GetNodeConnections(currentNode->GetIndex()))
			{
				...
				{
					// als node nog niet bezocht is, voeg toe aan queue om te bezoeken
		...
		// we reached to goal node, start backtracking
