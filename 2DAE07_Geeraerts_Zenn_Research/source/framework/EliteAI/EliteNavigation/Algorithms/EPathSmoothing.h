	...
	public:
		...
		static std::vector<Elite::Vector2> OptimizePortals(const std::vector<Portal>& portals)
		{
			...
			for (unsigned int i = 1; i < static_cast<unsigned int>(portals.size()); ++i)
			{
				...
				auto nextRightLeg{ portal.Line.p1 - apex };

				if (Elite::Cross(rightLeg, nextRightLeg) > 0)
				{
					//2. See if new line degenerates a line segment - RIGHT
					if (Elite::Cross(nextRightLeg, leftLeg) > 0)
					{
					...
					else
					{
						...
						int it{ apexIndex + 1 };
						i = leftLegIndex = rightLegIndex = it;
						vPath.push_back(apex);

						if (it < static_cast<unsigned int>(portals.size()))
						{
				...
				auto nextLeftLeg{ portal.Line.p2 - apex };

				if (Elite::Cross(leftLeg, nextLeftLeg) < 0)
				{
					//2. See if new line degenerates a line segment - LEFT
					if (Elite::Cross(nextLeftLeg, rightLeg) < 0)
					{
						leftLeg = nextLeftLeg;
						leftLegIndex = i;
					}
